    ...
    return provider.Consumer<SuperAdminBackendProvider>(
      builder: (context, db, child) {
        ...
        final webhookConfigured =
            allDevices.where((d) => d.webHookUrl.trim().isNotEmpty).length;
        final outdatedFirmware =
            allDevices.where((d) => d.firmwareVersion.trim().isEmpty).length;
              ...
              OpsPanel(
                ...
                child: Column(
                  children: [
                    if (_showFilters) ...[
                      _buildFiltersPanel(
                          context, devices.length, allDevices.length),
                      const SizedBox(height: 14),
                    ],
                    if (devices.isEmpty)
                      const OpsEmptyState(
