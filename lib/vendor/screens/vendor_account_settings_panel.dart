...
class VendorAccountSettingsPanel extends StatefulWidget {
  final String roleLabel;
  final String userName;
  final String userEmail;

  const VendorAccountSettingsPanel({
    super.key,
  ...
  @override
  State<VendorAccountSettingsPanel> createState() =>
      _VendorAccountSettingsPanelState();
}

class _VendorAccountSettingsPanelState
    extends State<VendorAccountSettingsPanel> {
  ...
  Future<void> _loadProfileForm() async {
    ...
    try {
      ...
      final userId = prefs.getString('vendor_principal_id')?.trim() ?? '';
      if (userId.isEmpty) {
  ...
  Future<void> _prepareThresholdsTab() async {
    final db = context.read<VendorBackendProvider>();
    db.loadSensors();
  ...
  Widget _buildThresholdsTab(BuildContext context) {
    ...
    final subColor =
        isLight ? const Color(0xFF4f6b82) : const Color(0xFF9db7d2);
    final db = context.watch<VendorBackendProvider>();
