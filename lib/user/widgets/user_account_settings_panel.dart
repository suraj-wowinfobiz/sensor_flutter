...
class UserAccountSettingsPanel extends StatefulWidget {
  ...
  @override
  State<UserAccountSettingsPanel> createState() =>
      _UserAccountSettingsPanelState();
}

class _UserAccountSettingsPanelState extends State<UserAccountSettingsPanel> {
  ...
  Future<void> _loadProfileForm() async {
    ...
    try {
      ...
      final userId = prefs.getString('user_principal_id')?.trim() ?? '';
      if (userId.isEmpty) {
  ...
  Widget _buildThresholdsTab(BuildContext context) {
    ...
    final subColor =
        isLight ? const Color(0xFF4f6b82) : const Color(0xFF9db7d2);
    final db = context.watch<UserDatabaseProvider>();
